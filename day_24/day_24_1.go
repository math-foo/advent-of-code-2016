...
func find_shortest_path(start_posn posn, remaining []posn, pair_map map[string]int) int {
	...
	if len(remaining) == 2 {
		...
		start_b, ok2 := pair_map[fmt.Sprintf("%d-%d", start_posn.id, remaining[1].id)]
		a_b, ok3 := pair_map[fmt.Sprintf("%d-%d", remaining[0].id, remaining[1].id)]

		if !(ok && ok2 && ok3) {
			fmt.Println("uh-oh triple")
		}

		min_3_path := 0
		if start_a < start_b {
			min_3_path = start_a + a_b
		} else {
			min_3_path = start_b + a_b
		}
		return min_3_path
	}
