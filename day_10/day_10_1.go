...
func activate(b *bot, bots map[int]*bot) []int {
	low := -1
	high := -1
	if b.chip1 < b.chip2 {
		low = b.chip1
...
func main() {
    ...
    special_bot_search := true
    special_bot_id := 10001
    for special_bot_search {
	    next_bots := []int{}
	    if len(active_bots) == 0 {
		    break
	    }
	    for _, active_bot := range active_bots {
		    if ((active_bot.chip1 == 61 || active_bot.chip2 == 61) &&
		        (active_bot.chip1 == 17 || active_bot.chip2 == 17)) {
				special_bot_search = false
				special_bot_id = active_bot.id
				break
		    }
		    these_bots := activate(active_bot, bots)
